...
public class FreeBoardController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(command.equals("/list.board01")) {//자유게시판 목록페이지로
			...
			String nickForderPath = rootPath + "sohee123";/*(String)request.getSession().getAttribute("nickname");*/
			...
			try {
				...
				for(FileItem fi : items) {
					...
					response.getWriter().append(/* (String)request.getSession().getAttribute("nickname") +*/ "sohee123/" + dateForderPath + "/" + tempFileName);
				}
			}catch(Exception e) {
				e.printStackTrace();
			}
			...
			String email = "email@naver.com";//(String)request.getSession().getAttribute("email");
