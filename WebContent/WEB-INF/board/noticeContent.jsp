...
<head>
        ...
        <script>
            $(function(){
                 $(".listBtn").on("click",function(){// 목록으로돌아가기
                	 location.href="list.board03?noticeCurrentPage=${noticeCurrentPage}";
                });
                 $(".deleteBtn").on("click",function(){ // 글 삭제 버튼
                	 var result = confirm("글을 삭제하시겠습니까?")
                	 
                	 if(result == true){
                		 location.href="deleteContent.board03?seq=${content.seq}&&noticeCurrentPage=${noticeCurrentPage}" ;
                         ...
                         /* $("img").each(function(i, item){
             				...
             				$.ajax({
                 				...
                 				console.log(resp)
                 				if(resp == "삭제성공"){
                         			location.href="deleteContent.board03?seq=${content.seq}" ;
                 				}else{
                 					location.href="deleteContent.board03?seq=${content.seq}" ;
                 				}
                 			});
                ...
                $(".alterBtn").on("click",function(){// 글 수정 버튼 -> 글 수정 페이지로 이동
                	location.href="alterForm.board03?seq=${content.seq}";
                })
               ...
               $(".commentBtn").on("click",function(){// 댓글 등록버튼 누르면
                	if(${type == null}){
        				...
        				if($(".comment-writeBox").val() == ""){
        					...
        					$.ajax({
                        		...
                        		location.href="wirteComment.board03?seq=${content.seq}&&commentPage=${cmCurrnetPage}";
                        	});
        				}
                ...
                $(".cmDeleteBtn").on("click",function(){ // 댓글삭제하기 
              			...
              			if(result == true){
                  			location.href="deleteComment.board03?seq="+seq+"&&postNum=${content.seq}";
              			}
              		});
                     ...
                     $(commentAlterBtn).on("click",function(){ // 댓글 수정에서 등록버튼 누르면 댓글입력창 숨기기                		
                 		if($(alterTextarea).val() == ""){
                 			alert("내용을 입력해주세요.");
                 			 ...
                 			 $.ajax({
                          		url:"alterComment.board03",
                          		data:{comment:$(alterTextarea).val(),seq:btnSeq}
                          	}).done(function(resp){
                          		if(resp == "수정됨"){
                          			$(alterTextarea).val("");
                          			$(".alterBox").hide();
                          			location.href="noticeContent.board03?seq=${content.seq}&&commentPage=${cmCurrnetPage}";
                          		}else if(resp == "수정안됨"){
        ...
        <style>
            *{margin:0 auto;}
            ...
            #wrapper{border: 0px solid black; position: relative; top: 250px; box-sizing: border-box;}
            ...
            .comment-writeBox{width: 100%; height: 100%;}
            .commentBtn{width: 100%; height: 100%; }
            .comment-Navi{text-align: center; background-color:;}
           
            .alterBox{ z-index:10; background-color: #ffc6e650; width: 100%; height: 100%; margin: 0; position: absolute;}
            .alterBox>div{padding: 0;}
            .alterBox input[type="button"]{width: 100%; height: 50%; }
            .alterTextarea{width: 100%; height: 100%;}
                    ...
                    <c:choose>
                        <c:when test="${email != content.email and type!=4 }">
                            <input type="button" value="수정" class="alterBtn btn btn-primary" hidden><!--글 수정버튼-->
                             <input type="button" value="삭제" class="deleteBtn btn btn-primary" hidden><!--글 삭제버튼-->
                        </c:when>
             ...
             <c:forEach var="list" items="${comList }">
               ...
               <div class="row  mb-5 alterBox" id="${list.seq }"> 
                   <div class="col-lg-11 col-md-9 col-sm-10 col-10">
                    <textarea name="alterTextarea" class="alterTextarea" maxlength="200" id="alterTextarea${list.seq }" seq=${list.seq } maxlength="200"></textarea>
                </div>
                <div class="col-lg-1 col-md-3 col-sm-2 col-2">
                     <input type="button" value="등록" seq=${list.seq } class="commentAlterBtn btn btn-primary" id="commentAlterBtn${list.seq }"> <!--댓글 수정 등록 버튼  -->
                    <input type="button" value="취소" seq=${list.seq } class="alterCancelBtn btn btn-primary" id="alterCancelBtn${list.seq }"> <!-- 댓글수정창 닫기 -->
                </div>
                </div>
                
                
                <div class="col-lg-6 col-md-6 col-sm-6 col-6 cmWriter">${list.writer }</div><!-- 댓글 작성자 -->
                <div class="col-lg-6 col-md-6 col-sm-6 col-6 cmBtn">
                   <c:choose>
               		<c:when test="${email != list.email and type != 4}">
               			<input type="button" class="cmAlterBtn btn btn-primary" value="수정" seq=${list.seq } hidden> <!--댓글 수정버튼-->
               			<input type="button" class="cmDeleteBtn btn btn-primary" value="삭제" seq=${list.seq }  hidden><!--댓글 삭제버튼-->
               		</c:when>
               		<c:when test="${email == list.email or type == 4}">
               			<input type="button" class="cmAlterBtn btn btn-primary" seq=${list.seq } value="수정">
               			<input type="button" class="cmDeleteBtn btn btn-primary" seq=${list.seq } value="삭제">
               		</c:when>
               	</c:choose>	
                </div>
                <div class="col-lg-12 col-md-12 col-sm-12 col-12 comment">${list.comment }</div> <!-- 댓글 내용 -->
                <div class="col-lg-12 col-md-12 col-sm-12 col-12">${list.timeForm }</div><!-- 댓글 등록 시간 -->
                <div class="line"></div>
            </div>
            </c:forEach>
