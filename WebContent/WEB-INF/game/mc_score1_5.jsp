			...
			<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 text-center h1 pb-1 font-weight-bold">
				...
				<div class="row my">
					<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 my">
						<form id="speakerForm">
							<div id="fake" style="display:none;">${beverages.get(Math.floor(Math.random()*(5+1))).menuName }</div>
...
<!----------machine view start----------------------------------------------->
				...
				<!-- 	우측메뉴 슬라이드 시작 -->
						...
						<script>
							$(function(){
								var beverageNameY;
								$('.device-slider__item:nth-child(4)').prepend("<span class='animated infinite bounce'></span>");
								$('.device-slider__item:nth-child(4)').on("click", function(){
									...
									$('.nextViewBtn').on('click', function(){
										$.ajax({
											url : "beverage_value.kiosk",
											type : "post",
											data : {key1 : $('#fake').html(), key2 : beverageNameY}
										}).done(function(resp){
											alert(resp);
										});
										$(location).attr('href', 'mc_slide.kiosk');
									});
								});
							});
						</script>
