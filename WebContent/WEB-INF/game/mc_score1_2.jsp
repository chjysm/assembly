...
/* 		(위쪽)수정하지 마세요 */
		.lcd{min-height:680px; border:30px solid #000; border-bottom:55px solid #000; border-radius:5px; box-shadow:0 0 15px #ddd; background:#0B6138;}
	...
	$(function(){
		$('.takeClick').on("click", function(){
			$.ajax({
				url : "takeio.kiosk",
				type : "post",
				data : {key1 : randomPick, key2 : $(this).find('p').text()}
			}).done(function(resp){
				alert(resp);
			});
			$(location).attr("href", "mc_mainMenu.kiosk");
		});
	});
	</script>
<!-- 스크립트 끝 -->
			...
			<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 p-3 my">
				<div class="row my">
					<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 my">
						<form id="speakerForm">
						    <textarea id="speakerTxt" class="text-center h3" style="width:100%; height:100%; border:none; background:none; display:block; overflow-x:auto;" disabled></textarea>
						    <script>
						    	var takeInOut = ["매장에서 식사", "테이크아웃(포장하기)"];
...
<!----------machine view start----------------------------------------------->
					...
					<script>
						var result = $('.takeClick').find('p');
						var tag="<span class='animated infinite bounce'><img src='Resources/img/pointerb.png' width='50' height='50'></span>";
						for(var i=0; i<$('.takeClick').length; i++){
							if(result[i].textContent==randomPick){
								$('.takeClick').find('i')[i].innerHTML=tag;
								break;
							}
						}
					</script>
